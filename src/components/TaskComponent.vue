    ...
    <el-card class="box_card">
      ...
      <div class="input_container">
        ...
        >
          ...
          <el-form-item style="margin-left: -120px">
            <el-col :span="11">
              <el-form-item label="工人数量" required>
                <el-input-number
                  v-model="task_form.workerNumTotal"
                  :min="1"
                  :max="20"
                  :step="1"
                  style="width: 200px"
          ...
          <el-form-item style="margin-left: -120px">
            <el-col :span="8">
              <el-form-item label="任务难度" required>
                <el-slider
                  v-model="task_form.taskDifficulty"
                  :step="1"
                  style="width: 200px"
                  :min="1"
                  :max="5"
          ...
          <el-form-item style="margin-left: 340px" v-if="!isSub">
            <el-button type="primary" @click="handleSubmit('basicInfo')"
              >提交任务</el-button
            >
  ...
  methods: {
    ...
    handleSubmit(formName) {
      ...
      console.log(formName);
        if (valid) {
          task = {
            requirementDescriptionFileList:
              this.task_form.requirementDescriptionFileList,
    ...
    async fnUploadRequest(option) {
      await oss.ossUploadFile(option).then((res) => {
        ...
        this.task_form.executableFileList.push(file);
        console.log(this.task_form.executableFileList);
      });
    },
    async fnUploadRequest1(option) {
      await oss.ossUploadFile(option).then((res) => {
        ...
        this.task_form.requirementDescriptionFileList.push(file);
        console.log(this.task_form.requirementDescriptionFileList);
      });
    },
    ...
    validEnd(rule, value, callback) {
      if (this.task_form.taskEndTime <= this.task_form.taskStartTime) {
        callback(new Error("任务结束时间不得早于开始时间"));
      }
      callback();
    },
    validFiles(rule, value, callback) {
      if (
        this.task_form.executableFileList.length === 0 ||
        this.task_form.requirementDescriptionFileList.length === 0
      ) {
        callback(new Error("请上传相关文件"));
      }
      callback();
